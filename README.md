...
- **聚合搜索**：同时搜索 QQ音乐 + 网易云 + 酷狗，结果去重合并，显示来源标签
- **下载源选择**：自动 / QQ / 网易云 / 酷狗 / 网页搜索，可指定下载来源
- **网页搜索**：直搜已知 mp3 站（歌曲宝等）+ Bing 搜索 + AI 探针分析页面
- **三层下载回退**：主音源 → 备选音源（精确歌名优先）→ 网页搜索（AI 辅助）
- **AI 音源发现**：支持 DeepSeek / OpenAI / Claude，自动搜索互联网 + 分析网页 + 注册新音源
...
本工具（QQMusicDL）是一个开源的音乐搜索与下载研究项目，仅供**技术学习与研究**使用。

**使用本工具即表示您同意以下条款：**

1. **非商业用途**：本工具仅限个人学习、研究使用，严禁用于任何商业目的或营利活动。
2. **版权归属**：所有音乐的版权归原始权利人（包括但不限于腾讯音乐、网易云音乐、酷狗音乐等平台及其版权方）所有。本工具不声称对所下载的任何内容拥有任何权利。
3. **用户责任**：您应自行确保对所下载的内容拥有合法权利（如个人备份、合理使用等）。下载受版权保护的音乐可能违反您所在地区的法律。您对使用本工具所产生的所有法律后果承担全部责任。
4. **不鼓励盗版**：本工具开发者和贡献者不鼓励、不支持任何形式的音乐盗版行为。如果您喜欢某首歌曲，请通过正版渠道购买或订阅支持艺术家。
5. **技术研究性质**：本项目旨在研究音乐平台的 API 设计、搜索引擎聚合、AI 辅助内容发现等技术领域。API 接口分析仅用于技术研究目的。
6. **无担保**：本软件按"原样"提供，不提供任何明示或暗示的担保，包括但不限于适销性、特定用途适用性的担保。
7. **停止使用**：如果您不同意以上条款，或您是任何被涉及的音乐平台的权利人，请立即停止使用本工具。

**Cookie 使用说明**：工具仅在本地存储您主动登录后的 Cookie 信息用于合法的个人访问。Cookie 数据不会上传至任何服务器，不会用于任何商业目的，且您可以随时清除。

**依赖说明**：本工具依赖的第三方库（FastAPI、requests、websocket-client 等）均为开源项目，各自保留其许可证权利。
